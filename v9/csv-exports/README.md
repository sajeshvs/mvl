...
- **Tax columns** added to GSA PO Workbench (`tax`, `taxUSD`, `netTotal`, `netTotalUSD`)
- **Tax columns** added to SM Quotation Workbench (`Tax`, `NetTotal`)
- **Tax columns** added to M&D Quotations & POs (`taxUSD`, `netTotalUSD`)
- **Tax KPIs** in GSA Summary (`totalTaxUSD`, `posWithTax`, `totalNetSpendUSD`)
- **Tax KPIs** in SM Summary (`totalQuotationTaxUSD`, `totalQuotationNetUSD`, `quotationsWithTax`)
- PO source changed from CSV to XLS (with Tax/Net Total columns)
- Quotation source includes Tax/Net Total fields from new XLS exports
...
|-------------|--------------|-------------|
| `gsa_data.json` | **Global Spend Analysis** (Orange tab) | 3,620 Purchase Orders with spend, tax, entities, materials, change orders |
| `sm_data.json` | **Supplier Marketplace** (Blue tab) | 3,941 RFQ Quotations with status, tax, clients, contacts |
| `md_data.json` | **Materials & Disciplines** (Dark Blue tab) | Combined RFQs + POs with tax by material/discipline |
...
|---|------|---------|-------------|
| 01 | `01_GSA_PO_Workbench.csv` | ~3,620 | **Main PO list** -- every PO with values, **tax/netTotal (USD)**, entities, materials, CO status |
| 02 | `02_GSA_Summary.csv` | 1 | Aggregate KPIs: total spend, **totalTaxUSD**, posWithTax, PO count, supplier count, CO stats |
...
| # | File | Records | Description |
|---|------|---------|-------------|
| 13 | `13_SM_Quotation_Workbench.csv` | ~3,941 | **Main quotation list** -- every RFQ with status, value, **Tax, NetTotal** |
| 14 | `14_SM_Summary.csv` | 1 | Aggregate KPIs: total quotations, POs, win rate, **totalQuotationTaxUSD**, quotationsWithTax |
...
| # | File | Records | Description |
|---|------|---------|-------------|
| 21 | `21_MD_Quotations.csv` | ~3,941 | RFQ records for M&D view (value + **taxUSD/netTotalUSD** in USD) |
| 22 | `22_MD_Purchase_Orders.csv` | ~3,620 | PO records for M&D view (value + **taxUSD/netTotalUSD** in USD) |
...
**GSA PO Workbench** -- 4 tax columns per PO record:
| Field | Type | Description |
|-------|------|-------------|
| `tax` | float | Tax amount in **original currency** (0 if not available) |
| `taxUSD` | float | Tax amount converted to **USD** |
| `netTotal` | float | Net total (value + tax) in **original currency** |
| `netTotalUSD` | float | Net total converted to **USD** |

**SM Quotation Workbench** -- 2 tax columns per quotation:
| Field | Type | Description |
|-------|------|-------------|
| `Tax` | float | Tax amount in original currency (0 if not available) |
| `NetTotal` | float | Net total in original currency |

**M&D Quotations & POs** -- 2 tax columns (USD-converted):
| Field | Type | Description |
|-------|------|-------------|
| `taxUSD` | float | Tax amount in USD |
| `netTotalUSD` | float | Net total in USD |

**Tax Statistics:**
- 870 of 3,620 POs have tax data (~$1.69M total tax USD)
- 872 of 3,941 quotations have tax data (~$1.58M total tax USD)

### Currency Conversion
All `valueUSD` / `poSpendUSD` / `quotedValue` / `taxUSD` / `netTotalUSD` fields are converted to USD:
...
**Formula:** `valueUSD = originalValue / FX_RATE` (same formula applied to tax and netTotal)
...
`entityCode` (e.g., "E6831") is parsed from PO number and mapped to entity name via a 29-entry mapping table.

### Supplier Count
- `supplierCount`: Total from `suppliers.json` master list (2,189)
- `activeSupplierCount`: Unique suppliers appearing in actual PO data

### SM Win Rate
`winRate = (totalPOs / totalQuotations) x 100`

### Material vs Material Code
- `material`: Specific material name (e.g., "Bare Copper Grounding Cable") -- 30+ names
- `materialCode`: Broader category (e.g., "Electrical") -- 12 categories

---

## Data Source Files (V9)

| File | Format | Rows | Key Columns |
|------|--------|------|-------------|
| `PO_List_*.xls` | XLS (Excel 97) | 3,637 | No, PO number, Po Date, PO Name, Supplier, Total, **Tax**, **Net Total**, Cur. |
| `Quotation_List_*.xls` (x5) | XLS (Excel 97) | 12,276 | No, Number, Company, Date, Type, Client, Project Name, Description, Material, Material Code, Quo. Value, **Tax**, **Net Total**, Cur., MVL Contact, Status |

All source files are in `v9/Full data of Quotations and POs with TAX fields/`.

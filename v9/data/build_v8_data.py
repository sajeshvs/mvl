"""
V9 Data Pipeline — Tax Fields + Dynamic Excel Integration
=========================================================
Reads Excel exports with Tax/Net Total fields from the
"Full data of Quotations and POs with TAX fields" folder.
Fully dynamic: handles any data size, auto-detects files,
...
Key capabilities:
  ...
  - Reads Tax and Net Total fields from updated exports
...
Input:  v9/Full data of Quotations and POs with TAX fields/*.xls
Output: v9/data/*.json (same structure scripts.js expects, with tax fields)
"""
...
CSV_DIR = os.path.join(PARENT_DIR, 'Data-New')  # CSV-based PO data source (v8 legacy)
TAX_DIR = os.path.join(PARENT_DIR, 'Full data of Quotations and POs with TAX fields')  # v9: XLS with Tax fields
...
def load_po_xls_tax():
    """Load PO data from PO_List_*.xls in the TAX_DIR folder.
    XLS has 9 columns: No, PO number, Po Date, PO Name, Supplier, Total, Tax, Net Total, Cur.
    Main Order ID and Order ID are derived from PO number parsing.
    """
    po_pattern = os.path.join(TAX_DIR, 'PO_List_*.xls')
    po_files = sorted(glob.glob(po_pattern))
    if not po_files:
        raise FileNotFoundError(f'No PO_List_*.xls found in {TAX_DIR}')
    po_file = po_files[-1]
    print(f'  Loading XLS with Tax: {os.path.basename(po_file)} (auto-detected)')
    ...
    PO_TAX_COLUMN_MAP = {
        'po_number':  ['PO number', 'PO No', 'PO Number'],
        'po_date':    ['Po Date', 'PO Date', 'Date'],
        'po_name':    ['PO Name', 'Name', 'Description'],
        'supplier':   ['Supplier', 'Vendor'],
        'total':      ['Total', 'Amount', 'Value'],
        'tax':        ['Tax', 'VAT', 'Tax Amount'],
        'net_total':  ['Net Total', 'Net Amount', 'Grand Total'],
        'currency':   ['Cur.', 'Currency', 'Cur'],
    }
    col, warnings = build_column_map(headers, PO_TAX_COLUMN_MAP)
    for w in warnings:
        print(f'  ⚠️ {w}')
    ...
    tax_col = col.get('tax', 6)
    net_col = col.get('net_total', 7)
    currency_col = col.get('currency', 8)
    ...
    for r in range(1, sheet.nrows):
        ...
        date_str = dt.strftime('%d %b %Y') if dt else cell_str(row_cells[date_col])

        po_parsed = parse_po_number(po_num)
        entity_code = po_parsed['entityCode']
        entity_name = ENTITY_CODE_MAP.get(entity_code, 'Unknown')
        code_prefix = po_parsed['codePrefix']
        material_from_prefix = PO_CODE_PREFIX_MAP.get(code_prefix, 'Various')
        main_order_id = po_parsed['mainOrderId']
        order_id = po_parsed['baseGroupKey']

        total_val = cell_float(row_cells[total_col])
        tax_val = cell_float(row_cells[tax_col]) if tax_col >= 0 else 0.0
        net_val = cell_float(row_cells[net_col]) if net_col >= 0 else total_val
    ...
    print(f'  Loaded {len(records)} PO records from XLS (with Tax)')
    return records


def load_quotation_xls_tax():
    """Load Quotation data from fragment files in TAX_DIR.
    New XLS has 16 columns including Tax and Net Total.
    Uses ignore_workbook_corruption=True for large files.
    Filters to RFQ only (no IQ records).
    """
    fragments = sorted([
        f for f in os.listdir(TAX_DIR)
        if f.startswith('Quotation_Report_') and f.endswith('.xls')
    ])
    if not fragments:
        print('  ⚠️ No Quotation_Report_*.xls files found in TAX_DIR!')
        return []
    print(f'  Found {len(fragments)} quotation fragment files (with Tax)')

    Q_TAX_COLUMN_MAP = {
        'number':        ['Number', 'Quo. Number', 'Quotation Number'],
        ...
        'material_code': ['Material Code', 'Mat. Code'],
        'value':         ['Quo. Value', 'Value', 'Amount'],
        'tax':           ['Tax', 'VAT', 'Tax Amount'],
        'net_total':     ['Net Total', 'Net Amount', 'Grand Total'],
        'currency':      ['Cur.', 'Currency', 'Cur'],
        'contact':       ['MVL Contact', 'Contact'],
    ...
    for fname in fragments:
        ...
        if col is None:
            col, warnings = build_column_map(headers, Q_TAX_COLUMN_MAP)
            for w in warnings:
                print(f'    ⚠️ {w}')
        ...
        tax_col = col.get('tax', 11)
        net_col = col.get('net_total', 12)
        curr_col = col.get('currency', 13)
        contact_col = col.get('contact', 14)
        status_col = col.get('status', 15)
        ...
        for r in range(data_start, sheet.nrows):
            ...
            tax_val = cell_float(row_cells[tax_col]) if tax_col >= 0 else 0.0
            net_val = cell_float(row_cells[net_col]) if net_col >= 0 else quo_value
    ...
    print(f'  Loaded {len(all_records)} total quotation records (with Tax)')
    return all_records
...
def main():
    print('=' * 60)
    print('V9 Data Pipeline — Tax Fields + Dynamic Excel Integration')
    print('=' * 60)

    # ── 1. Load data ─────────────────────────────────────────
    print('\n[1/9] Loading data files...')
    # V9: Prefer TAX_DIR (XLS with Tax/Net Total); fallback to CSV then legacy XLS
    tax_po_pattern = os.path.join(TAX_DIR, 'PO_List_*.xls')
    tax_q_pattern = os.path.join(TAX_DIR, 'Quotation_Report_*.xls')
    if glob.glob(tax_po_pattern):
        print('  Using TAX_DIR source (Full data with TAX fields)')
        raw_pos = load_po_xls_tax()
        has_tax = True
    else:
        csv_pattern = os.path.join(CSV_DIR, 'PO_List_*.csv')
        if glob.glob(csv_pattern):
            print('  TAX_DIR not found, using CSV source (Data-New/)')
            raw_pos = load_po_csv()
        else:
            print('  CSV not found, falling back to legacy XLS')
            raw_pos = load_po_excel()
        has_tax = False
        # Add zero-value tax fields for backward compat
        for po in raw_pos:
            po.setdefault('tax', 0.0)
            po.setdefault('netTotal', po.get('originalValue', 0.0))

    if glob.glob(tax_q_pattern):
        print('  Loading quotations from TAX_DIR')
        raw_quotes = load_quotation_xls_tax()
    else:
        print('  Loading quotations from legacy EXCEL_DIR')
        raw_quotes = load_quotation_excel()
        for q in raw_quotes:
            q.setdefault('Tax', 0.0)
            q.setdefault('NetTotal', q.get('QuotationValue', 0.0))
    ...
    for po in raw_pos:
        ...
        po['taxUSD'] = to_usd_po(po.get('tax', 0), po['currency'])
        po['netTotalUSD'] = to_usd_po(po.get('netTotal', po['originalValue']), po['currency'])
    ...
    total_quote_tax = sum(to_usd(q.get('Tax', 0), q['Currency']) for q in clean_quotes)
    total_quote_net = sum(to_usd(q.get('NetTotal', q['QuotationValue']), q['Currency']) for q in clean_quotes)
    quotes_with_tax = sum(1 for q in clean_quotes if q.get('Tax', 0) > 0)
    ...
    sm_data = {
        ...
        'summary': {
            ...
            'totalQuotationTaxUSD': round(total_quote_tax, 2),
            'totalQuotationNetUSD': round(total_quote_net, 2),
            'quotationsWithTax': quotes_with_tax,
        },
        'statusSummary': [
    ...
    # Tax aggregates for GSA
    total_tax_usd = sum(po.get('taxUSD', 0) for po in clean_pos)
    total_net_usd = sum(po.get('netTotalUSD', 0) for po in clean_pos)
    pos_with_tax = sum(1 for po in clean_pos if po.get('taxUSD', 0) > 0)
    ...
    gsa_data = {
        ...
        'summary': {
            ...
            'totalTaxUSD': round(total_tax_usd, 2),
            'totalNetSpendUSD': round(total_net_usd, 2),
            'posWithTax': pos_with_tax,
        },
    ...
    for q in clean_quotes:
        ...
        tax_usd = to_usd(q.get('Tax', 0), q['Currency'])
        net_usd = to_usd(q.get('NetTotal', q['QuotationValue']), q['Currency'])
        md_q = {
    ...
    for po in clean_pos:
        md_po = {
            ...
            'value': po['poSpendUSD'],
            'taxUSD': po.get('taxUSD', 0),
            'netTotalUSD': po.get('netTotalUSD', 0),
            'currency': 'USD',
            'year': po.get('year'),
            'month': po.get('month'),
    ...
    print('\n' + '=' * 60)
    print('V9 DATA PIPELINE COMPLETE')
    print('=' * 60)
    ...
    print(f'  POs with Tax:   {pos_with_tax}')
    print(f'  Total Tax (USD): ${total_tax_usd:,.2f}')
    print(f'M&D Quotations:   {len(md_quotations)}')
    print(f'M&D POs:          {len(md_pos)}')
    ...
    print(f'Tax data:         {"YES" if has_tax else "NO"}')

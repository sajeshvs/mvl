...
- **Root cause:** SM sourced PO count from `smData.summary.totalPOs` (3,613 — quotation-linked POs only) while GSA sourced from actual PO records (3,620)
- **Fix:** Changed `enrichDashboardWithRealData()` to source `poCount` from `gsaData.summary.totalPOs` instead of `smData.summary.totalPOs`
- **Filter fix:** `applyFilters()` now cross-filters `gsaState.allPOs` by active SM filters for dynamic PO count
- **Files changed:** `v9/shared/scripts.js` — `enrichDashboardWithRealData()` (~L300) and `applyFilters()` (~L1519)
...
- **Root cause:** SM sourced PO spend from `smData.summary.totalPOSpendUSD` (quotation-linked subset) while GSA sourced from all POs
- **Fix:** Changed `enrichDashboardWithRealData()` to source `poValue` from `gsaData.summary.totalSpendUSD`
- **Filter fix:** `applyFilters()` computes filtered PO spend from `gsaState.allPOs` cross-filtered by SM filters
- **Files changed:** `v9/shared/scripts.js` — same locations as Q1
...
- **Root cause:** SM used `smData.summary.changeOrders` (rounded differently) while GSA computed from PO records
- **Fix:** SM now sources CO count/value from `gsaData.summary.changeOrders` for consistency
- **Filter fix:** When filters active, CO count/value computed dynamically from filtered GSA POs (`po.poType === 'Change Order'`)
- **Files changed:** `v9/shared/scripts.js` — `enrichDashboardWithRealData()` and `applyFilters()`
...
- **Fix:** Computed `quoteValueWithTax = totalQuotationValueUSD + totalQuotationTaxUSD` and displayed combined total
- **Subtext:** Still shows "Tax: $X.XXM" for transparency
- **Filter fix:** `applyFilters()` computes `totalQuoteTax` separately and adds to `totalQuoteValue`
- **Files changed:** `v9/shared/scripts.js` — `enrichDashboardWithRealData()` and `applyFilters()`
...
- **Root cause:** `formatCurrency()` used `.toFixed(1)` for M/B suffixed values; `updateKPIs()` called `formatCurrency()` instead of `formatCurrencyShort()`
- **Fix:** Changed `formatCurrency()` to use `.toFixed(2)` for all ranges. Changed `updateKPIs()` to use `formatCurrencyShort()` for KPI rendering
- **Files changed:** `v9/shared/scripts.js` — `formatCurrency()` (~L2855) and `updateKPIs()`
...
- **Status:** Cannot implement until Vijay provides new data extract with "Quotation Closed" status records
- **Pipeline ready:** `build_v8_data.py` normalize_status() will handle new status once data is available
- **No code changes needed** — status chart and filter dynamically read from data
...
- **Root cause:** PO/CO KPIs used static `gsaData.summary` values instead of filtering from raw PO data
- **Fix:** Added cross-filtering logic in `applyFilters()` — filters `gsaState.allPOs` by all active `currentFilters` (entity, project, supplier, material, materialCode, dateFrom, dateTo, search)
- **Dynamic values:** PO count, PO value, CO count, CO value all recomputed from filtered GSA POs
- **Win Rate fix:** Now computed as `filteredPOCount / filteredRFQCount` using consistent filtered data
- **Files changed:** `v9/shared/scripts.js` — `applyFilters()` (~L1549)
...
- **Fix:** Removed Bar/Line/Radar toggle buttons from `index.html`, renamed chart title to "Discipline Distribution"
- **Default chart type:** Changed from `'bar'` to `'pie'` in `renderMaterialChartCanvas()` signature
- **Chart toggle handler:** Forces `'pie'` type on all calls
- **Files changed:** `v9/index.html` (toggle buttons removed, title renamed) and `v9/shared/scripts.js` (chart type defaults)
...
- **Root cause:** `generateSupplierListRowsPaginated()` only used its own local filters (search, material, country), not top-level SM `currentFilters`
- **Fix:** Added cross-filtering: when top-level filters are active, filters `smData.workbench` by entity/project/supplier/material/materialCode/status, builds `topFilteredSupplierNames` Set from Client field, and filters supplier list to only matching names
- **Files changed:** `v9/shared/scripts.js` — `generateSupplierListRowsPaginated()` (~L1007)
...
- **Status:** Already working as designed. Map has 60+ `countryCoords` entries covering all supplier countries.
- **Filter updates:** Map rendering is called in `applyFilters()` with `fitBounds` for filtered data
- **Country normalization:** `normalizeCountry()` (~150 entries) applied on all tabs
- **No code changes needed** — verified existing implementation is correct
...
- **Status:** Resolved through individual component fixes:
  - **A) Material Distribution:** Already updates via `applyFilters()` → `renderMaterialChartCanvas()` with filtered data
  - **B) Quotation to PO Time:** Fixed in Q12 — cross-references `conversion_times.json` records with filtered workbench
  - **C) Submit & Order Quantity:** Fixed in Q13 — builds trend from filtered workbench + GSA data
  - **D) Supplier List:** Fixed in Q9 — cross-filters with `currentFilters`
  - **D) Quotation Details:** Already filters correctly via `applyFilters()`
...
- **Root cause:** Chart used pre-calculated `monthlyAverage` from `conversion_times.json`, filtered only by date range
- **Fix:** Replaced with raw `records` filtering — cross-references `conversion_times.records` with filtered workbench quotation numbers
- **Logic:** When filters active, builds `filteredQuotationNumbers` Set from filtered workbench, filters records by both date range AND quotation number membership, then recomputes monthly averages
- **Files changed:** `v9/shared/scripts.js` — `applyFilters()` (~L1838)
...
- **Root cause:** `renderTrendChartLine()` not called in `applyFilters()` — always showed full unfiltered data
- **Fix:** Added monthly trend computation in `applyFilters()` using filtered workbench data (quotes/orders per month) plus CO counts from `filteredGSAPOs`
- **Logic:** Builds `trendMonthlyMap` from filtered data, sorts by month, calls `renderTrendChartLine(trendData)` with sorted monthly array
- **Files changed:** `v9/shared/scripts.js` — `applyFilters()` (~L1712)
...
- **Root cause:** Unfiltered code path used `gsaData.monthlyTrend` which had no Base/CO split, setting `changeData = monthlyTrend.map(() => 0)`
- **Fix:** Removed the branch that zeroed out CO data. Now ALWAYS computes Base/CO split from raw PO data using `po.poType === 'Change Order'` check
- **Files changed:** `v9/shared/scripts.js` — `createGSASpendTrendChart()` (~L3700)
...
- **Fix:** Added `id="gsaBottomSuppliersCard"` to the card div in `index.html`
- **Logic:** At top of `createGSASupplierCharts()`, checks `isFiltered` flag and sets `bottomCard.style.display = isFiltered ? 'none' : ''`
- **Files changed:** `v9/index.html` (card id) and `v9/shared/scripts.js` (visibility toggle)
...
- **Root cause:** PO enrichment used `orderId` matching (which was empty for most quotations), so 0 POs were enriched with raw material names
- **Pipeline fix:** Changed `build_v8_data.py` step 5 to use `mainOrderId` matching with `orderId` fallback. Prefers quotation with matching `materialCode` for more accurate material assignment
- **Result:** 3,453 of 3,620 POs now enriched with raw material names from linked quotations
- **GSA materials:** Now 23 distinct values (was 12 = identical to materialCodes)
- **M&D materials:** 28 distinct values
- **SM materials:** 27 distinct values (was already correct from quotation data)
- **Files changed:** `v9/data/build_v8_data.py` (step 5 enrichment logic), data regenerated
...
- **Fix:** Added `deduplicateCOGroups(pos)` helper function that for each orderId group with `changeOrderTotal > 1`, keeps only the latest version (highest `poVersion`)
- **Applied in:** `createGSAEntityChart()`, `createGSAProjectChart()`, `createGSASupplierCharts()` filtered paths
- **Logic:** Groups POs by orderId, identifies CO groups, keeps only the PO with highest poVersion per group
- **Files changed:** `v9/shared/scripts.js` — new `deduplicateCOGroups()` helper (~L3872) and chart functions
...
- **Fix:** In `updateGSATable()`, computed `valueWithTax = valueInUSD + taxUSD` and used that for the PO VALUE column display
- **Files changed:** `v9/shared/scripts.js` — `updateGSATable()` (~L4410)
...
- **Rename:** Changed all M&D tab references from "Material Code" to "Discipline(s)":
  - `index.html`: Filter label, KPI labels ("DISCIPLINES", "TOTAL DISCIPLINE SPEND"), chart title ("Total Spend by Discipline"), table header
  - `scripts.js`: Filter dropdown default text ("All Disciplines"), filter comparison values in `applyMdFilters()`
- **Filter function:** `applyMdFilters()` already filters by both discipline and material — verified working correctly
- **Files changed:** `v9/index.html` (6 text changes) and `v9/shared/scripts.js` (3 filter string changes)
...
- **Cascading filter (A):** Added `updateMdMaterialDropdown()` function that repopulates the Material dropdown based on selected Discipline
  - Builds `disciplineToMaterials` mapping from actual PO/quotation data at init time
  - On Discipline change, filters Material dropdown to only show materials belonging to that discipline
  - Preserves current material selection if still valid after cascade
  - Refreshes `SearchableSelect` wrapper if present (via `_searchableSelect.refresh()`)
  - Also called on `clearMdFilters()` to reset to all materials
- **Data pipeline fix (B):** Resolved in Q16 — `material` and `materialCode` fields now contain distinct values after pipeline enrichment
- **SearchableSelect:** Added `this.select._searchableSelect = this` reference in constructor for external refresh
- **Files changed:** `v9/shared/scripts.js` (new `updateMdMaterialDropdown()`, modified `initMdFilters()`, `clearMdFilters()`, `SearchableSelect` constructor)
...
| Question                                                                                     | Status                                                              |
| -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Q1: PO Count (3,613 vs 3,620)                                                                | ✅ Resolved — SM now sources from gsaData.summary                   |
| Q2: PO Values ($412.6M vs $414.34M)                                                          | ✅ Resolved — SM now sources from gsaData.summary                   |
| Q3: CO Values ($12.0M vs $11.99M)                                                            | ✅ Resolved — SM now sources from gsaData.summary                   |
| Q4: Quote Value — include tax in total ($259.87M)                                            | ✅ Resolved — Quote Value = net + tax combined                      |
| Q5: Decimal precision — standardize to 2 decimals                                            | ✅ Resolved — formatCurrency uses .toFixed(2)                       |
| Q6: Missing 5th status "Quotation Closed"                                                    | ⏳ BLOCKED — Requires new data extract from Vijay                   |
| Q7: PO/CO KPIs not updating on filter                                                        | ✅ Resolved — Cross-filters gsaState.allPOs by SM filters           |
| Q8: Material Distribution → Pie only, rename to "Discipline Distribution", mapping reference | ✅ Resolved — Default pie, renamed, toggle removed                  |
| Q9: Supplier List not filtering by Project/Entity                                            | ✅ Resolved — Cross-filters with currentFilters via workbench       |
| Q10: Map not showing all countries & not updating on supplier filter                         | ✅ Resolved — Already working (60+ countryCoords, fitBounds)        |
| Q11: Multiple components not updating on supplier filter (A-D)                               | ✅ Resolved — Via Q9, Q12, Q13 fixes                                |
| Q12: Quotation to PO Time not updating on any filter                                         | ✅ Resolved — Filters raw records via quotation cross-reference     |
| Q13: Submit & Order Quantity not updating on any filter                                      | ✅ Resolved — Builds trend from filtered workbench+GSA data         |
| Q14: Annual Spend Trend — CO values showing $0 across all months                             | ✅ Resolved — Always computes Base/CO split from raw POs            |
| Q15: Most Inactive Suppliers — hide when any filter applied                                  | ✅ Resolved — Card hidden when isFiltered=true                      |
| Q16: Material & Material Code filters show same data                                         | ✅ Resolved — Pipeline uses mainOrderId matching, 3453 POs enriched |
| Q17: PO spend charts counting COs at full value, not incremental                             | ✅ Resolved — deduplicateCOGroups keeps latest version only         |
| Q18: GSA PO table values should include tax                                                  | ✅ Resolved — Table cell shows valueInUSD + taxUSD                  |
| Q19: M&D Material filter not updating data + rename "Material Code" to "Disciplines"         | ✅ Resolved — HTML+JS renamed, filters work                         |
| Q20: Material filter not cascading from Discipline + Material=Discipline data issue          | ✅ Resolved — Cascading dropdown + pipeline fix (Q16)               |

---

_Created: March 2, 2026_
_Resolutions completed: March 3, 2026_
